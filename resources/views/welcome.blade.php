...
<script>
    const eventSource = new EventSource('/comments/1/sse');
    const arr = [];
    eventSource.addEventListener('message', event => {
        const comment = JSON.parse(event.data);
        const newObj = comment.data;

        const exists = arr.some(obj => obj.id === newObj.id);

        if (!exists) {
            arr.push(newObj);
            commentElement = document.getElementById('div22');
            commentElement.innerText = commentElement.innerText + `
        <p>${comment.data.text}</p>
        <p>${comment.data.id}</p>
        <small>Posted by User ${comment.data.user_id} at ${comment.data.created_at}</small>
    `;
